read.csv(file = ("??nvolvement.csv"), sep = ";",  header = T)
??nvolvement <- read.csv(file = ("??nvolvement.csv"), sep = ";",  header = T)

library("caret")
partition <- createDataPartition(??nvolvement$INVOLVEMENT,p=0.75,list = FALSE)
train_data <- ??nvolvement[partition,]
test_data <-??nvolvement[-partition,]

library("mccr")
mccr(??nvolvement$INVOLVEMENT, testPC)
...
mccr(??nvolvement$INVOLVEMENT, testrpart)
summary(rpart)
plot(varImp(rpart))
...
mccr(??nvolvement$INVOLVEMENT, testwsrf)
summary(wsrf)
plot(varImp(wsrf))
wsrf$finalModel
...
mccr(??nvolvement$INVOLVEMENT, testadaboost)
summary(adaboost)
...
mccr(??nvolvement$INVOLVEMENT, testxgboost)
summary(xgboost)
plot(varImp(xgboost))
xgboost$finalModel
...
mccr(??nvolvement$INVOLVEMENT, testrf)
summary(rf)
plot(varImp(rf))
rf$finalModel$ntree
...
read.csv(file = ("v??.csv"), sep = ";",  header = T)
tablo1 <- read.csv(file = ("v??.csv"), sep = ";",  header = T)
